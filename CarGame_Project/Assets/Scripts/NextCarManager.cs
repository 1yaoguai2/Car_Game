...
public class NextCarManager : MonoBehaviour
{
    ...
    void CreateNewCar()
    {
        ...
        newCar1.GetComponent<CarController>().moveSpeed = ManagerModel.WallMoveSpeed * Random.Range(1.1f, 1.2f);
        index = Random.Range(0, cars.Count);
        var newCar2 = Instantiate<GameObject>(cars[index], new Vector3(carPosX == 1 ? 7f : 0f, 0, carPos + 10), Quaternion.Euler(new Vector3(0, 180, 0)));
        newCar2.GetComponent<CarController>().moveSpeed = ManagerModel.WallMoveSpeed * Random.Range(1f, 1.2f);
    }
}
